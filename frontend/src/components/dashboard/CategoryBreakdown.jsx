...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div className="bg-linen border border-stone/20 rounded-xl px-4 py-2.5 shadow-elevated">
      <p className="text-xs text-drift">{payload[0].name}</p>
  ...
  return (
    ...
    >
      <Card className="h-full flex flex-col" hover={false}>
        <div className="mb-4">
          <h3 className="text-base font-semibold text-obsidian">By Category</h3>
          <p className="text-xs text-drift mt-1">Expense distribution</p>
        </div>
          ...
          {!hasData && (
            <div className="absolute inset-0 flex items-center justify-center z-10">
              <div className="text-center">
                ...
                <p className="text-sm font-medium text-char">No categories yet</p>
                <p className="text-xs text-drift mt-1">Start spending to see breakdown</p>
              </div>
            </div>
          )}
          {hasData && (
        ...
        {/* Legend */}
        <div className="mt-auto pt-4 space-y-2.5 border-t border-stone/15">
          {data.slice(0, 5).map((item, i) => (
            ...
            >
              <div className="flex items-center gap-2.5">
                ...
                />
                <span className="text-sm text-char">{item.name}</span>
              </div>
              <span className="text-sm font-medium text-obsidian tabular-nums">
